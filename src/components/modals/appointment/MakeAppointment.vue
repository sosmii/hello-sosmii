  ...
  <div class="modal-content-wrapper">
    ...
    <div class="apo-modal__body">
      ...
      <transition mode="out-in">
        <div class="loading-icon-wrapper" v-if="!responseReceived" key="icon">
          <LineSpinFadeLoader/>
        </div>
        <div v-if="responseReceived" key="content">
          <div class="meeting-icontext-wrapper">
            <div class="meeting-icontext-wrapper__col">
              <label>
                <input type="radio" name="radio" class="fake-radio" @click="selectSkype()" :disabled="!responseReceived"/>
                <font-awesome-icon :icon="['fab', 'skype']" class="col__icon"/>
              </label>
              <div class="col__text">Skype</div>
            </div>
            <div class="meeting-icontext-wrapper__col">
              <label>
                <input type="radio" name="radio" class="fake-radio" @click="selectOffice()" :disabled="!responseReceived"/>
                <font-awesome-icon :icon="['fas', 'building']" class="col__icon"/>
              </label>
              <div class="col__text">オフィスにおいで</div>
            </div>
            <div class="meeting-icontext-wrapper__col">
              <label>
                <input type="radio" name="radio" class="fake-radio" @click="selectLunch()" :disabled="!responseReceived"/>
                <font-awesome-icon :icon="['fas', 'utensils']" class="col__icon"/>
              </label>
          ...
          <div class="row">
            <el-input v-model="place" placeholder="場所" style="width: 220px;" :disabled="!iconsSelected || apoType === 'skype'"/>
          </div>
          <button class="button" @click="makeAppointment(); showLoadingIcon()" :disabled="disabledCondition" v-if="!whileLoading">
...
export default {
  ...
  methods: {
    async getReservedDates () {
      const func = firebase.functions().httpsCallable('getReservedDates')
    ...
    async makeAppointment () {
      const func = firebase.functions().httpsCallable('makeAppointment')
